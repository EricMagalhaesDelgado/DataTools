...
function specData = Fcn_MetaDataReader(rawData, filename)
    ...
    specData.TaskData.Name        = MetaStruct.Task;
    specData.TaskData.ID          = MetaStruct.ID;    
    specData.TaskData.Description = MetaStruct.Description;
    ...
    specData.RelatedFiles(end+1,:) = {[file ext], BeginTime, EndTime, specData.Samples, RevisitTime};
...
function specData = Fcn_SpecDataReader(specData, rawData)
    ...
    if specData.FileMap.attData.Mode
        specData.FileMap.attData.Array = zeros(specData.Samples, 1, 'uint8');
    end
